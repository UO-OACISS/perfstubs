...
def init_tracing():
    ...
    sys.monitoring.set_events(PROFILER_ID, \
    ...
    def pstubs_py_throw(code, instruction_offset, exception):
        ...
        # Yes! Unintuitively, we want to START a timer on a throw.
    ...
    def pstubs_py_start_bootstrap(code, instruction_offset):
        ...
        if code.co_name in ps.bootstrap_timers:
            ...
            sys.monitoring.register_callback(PROFILER_ID, \
                sys.monitoring.events.PY_START, pstubs_py_start)
            sys.monitoring.register_callback(PROFILER_ID, \
                sys.monitoring.events.PY_RESUME, pstubs_py_resume)
            sys.monitoring.register_callback(PROFILER_ID, \
                sys.monitoring.events.PY_RETURN, pstubs_py_return)
            sys.monitoring.register_callback(PROFILER_ID, \
                sys.monitoring.events.PY_YIELD, pstubs_py_yield)
            sys.monitoring.register_callback(PROFILER_ID, \
                sys.monitoring.events.PY_UNWIND, pstubs_py_unwind)
            sys.monitoring.register_callback(PROFILER_ID, \
                sys.monitoring.events.PY_THROW, pstubs_py_throw)
            sys.monitoring.register_callback(PROFILER_ID, \
                sys.monitoring.events.RAISE, pstubs_raise)
            sys.monitoring.register_callback(PROFILER_ID, \
                sys.monitoring.events.STOP_ITERATION, pstubs_py_stop_iteration)
            sys.monitoring.register_callback( PROFILER_ID, \
                sys.monitoring.events.EXCEPTION_HANDLED, \
                pstubs_py_exception_handled)
            # Not sure about these two events yet...
            #sys.monitoring.register_callback(PROFILER_ID,
            #    sys.monitoring.events.CALL, pstubs_call_trace)
            #sys.monitoring.register_callback(PROFILER_ID,
            #    sys.monitoring.events.C_RETURN, pstubs_c_return_trace)
            if (rc == False):
                return sys.monitoring.DISABLE
    ...
    def pstubs_call_trace(code, instruction_offset, callable, arg0):
        frame = sys._getframe(1)
        perfstubs.start(code.co_name, os.path.basename(code.co_filename), \
                        frame.f_lineno)
...
def fini_tracing():
    ...
    sys.monitoring.register_callback(PROFILER_ID, \
        sys.monitoring.events.PY_START, None)
    sys.monitoring.register_callback(PROFILER_ID, \
        sys.monitoring.events.PY_RESUME, None)
    sys.monitoring.register_callback(PROFILER_ID, \
        sys.monitoring.events.PY_RETURN, None)
    sys.monitoring.register_callback(PROFILER_ID, \
